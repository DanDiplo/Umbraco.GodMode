...
import type { GodModeLogEvent, GodModeLogOverview, Page } from "../shared/types";
...
export class GodModeLogBrowserElement extends UmbElementMixin(LitElement) {
            ...
            <godmode-page heading="Log Browser" description="Inspect Umbraco JSON logs and explain individual entries with AI." show-reload @reload=${() => void this._load()}>
                ...
                <uui-box>
                    <form class="filters" @submit=${this._submitFilters}>
                        ...
                        <div>
                            <label>Level</label>
                            <select @change=${(e: Event) => (this._level = (e.target as HTMLSelectElement).value)}>
    ...
    static override styles = css`
        ...
        .filters {
            display: grid;
            grid-template-columns: minmax(220px, 2fr) minmax(130px, 0.6fr) minmax(130px, 0.6fr) minmax(130px, 0.6fr) max-content;
            gap: var(--uui-size-space-3);
            align-items: end;
        }
        label {
